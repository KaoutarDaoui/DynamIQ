...
from .rc import FitResult, fit_rc, one_step_ahead_rmse
...
def evaluate_calibration(t_measured_c: np.ndarray, t_ext_c: np.ndarray, q_solar_w: np.ndarray, q_occ_w: np.ndarray, q_hvac_w: np.ndarray, r_bounds: tuple[float, float], c_bounds: tuple[float, float], r_initial_guess: float, c_initial_guess: float, previous_rmse_c: float | None, train_fraction: float=0.7) -> tuple[FitResult, float, bool, str]:
    ...
    validation_rmse = one_step_ahead_rmse(fit.r_k_per_w, fit.c_j_per_k, t_measured_c[split:], t_ext_c[split:], q_solar_w[split:], q_occ_w[split:], q_hvac_w[split:])
...
def calibrate_room(engine: Engine, room_id: str, window_days: int=21, now: datetime | None=None) -> CalibrationOutcome:
...
def run_calibration_sweep(engine: Engine, building_id: str, window_days: int=21) -> list[CalibrationOutcome]:
    outcomes = []
    for room_id in fetch_instrumented_room_ids(engine, building_id):
        try:
            outcomes.append(calibrate_room(engine, room_id, window_days=window_days))
