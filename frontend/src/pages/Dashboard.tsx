...
import { fetchOrgBuildings, fetchAnomalies, fetchReportsSummary, fetchThermalModels, fetchMpcRooms, ThermalApiError } from "../lib/api";
...
const METRICS: { id: "energy" | "carbon"; label: string; color: string; unit: string }[] = [
...
function Kpi({ label, value, risk }: { label: string; value: string; risk: RiskLevel }) {
...
export default function Dashboard() {
  ...
  const [building, setBuilding] = useState<{ name: string; address: string; floors: number; rooms: number } | null>(null);
  const [calibratedCount, setCalibratedCount] = useState(0);
  const [instrumentedCount, setInstrumentedCount] = useState(0);
  const [mpcRooms, setMpcRooms] = useState<{ roomId: string; latestSolvedAt: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const realBuilding = fetchOrgBuildings()
      .then((list) => {
        ...
        if (b) setBuilding({ name: b.name, address: b.address ?? "-", floors: b.total_floors, rooms: b.rooms_count });
      })
      .catch(() => undefined);
  ...
  const latestMpcSolve = mpcRooms.length ? mpcRooms.map((r) => r.latestSolvedAt).sort((a, b) => +new Date(b) - +new Date(a))[0] : null;
    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-ink-400">Building telemetry, AI model health and live anomaly feed — real agent data, not mocked.</p>
        </div>
        <button
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-[22px] font-medium">{building?.name ?? (loading ? "Loading…" : buildingId)}</h1>
...
{building ? `${building.address} · ${building.floors} Floors · ${building.rooms} Rooms` : "Fetching building metadata…"}
            </p>
            <p className="mt-2 text-[12px] text-ink-400">Live from the orchestrator & thermal agent — not mocked.</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Kpi
          ...
          risk={openAnomalies.length === 0 ? "safe" : openAnomalies.length <= 2 ? "warn" : "danger"}
        />
        <Kpi
          label="Avg Comfort Deviation"
          value={loading ? "—" : avgDeviation !== null ? `${avgDeviation.toFixed(1)}°C` : "—"}
          risk={avgDeviation === null ? "safe" : avgDeviation <= 1 ? "safe" : avgDeviation <= 2 ? "warn" : "danger"}
        />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader title={chartTitle} subtitle="Predicted energy & carbon from Agent 2's real MPC solves" />
          ...
          {range === "custom" && (
            <div className="flex items-center gap-3 px-5 pb-3 text-[12px] text-ink-400">
              ...
              <span className="text-ink-400">Showing last 30 days of MPC predictions</span>
            </div>
          )}

          <div className="h-56 px-2 pb-4">
            {loading ? (
              <div className="flex h-full items-center justify-center text-[13px] text-ink-400">Loading telemetry…</div>
            ) : chartData.length === 0 ? (
              <div className="flex h-full items-center justify-center text-[13px] text-ink-400">No MPC predictions in this window yet.</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ left: 0, right: 12, top: 8, bottom: 0 }}>
                  ...
                  <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "#8c897d" }} width={48} />
                  <Tooltip
                    contentStyle={{
                      borderRadius: 12,
        ...
        <div className="flex flex-col gap-5">
          <Card>
            <CardHeader title="AI model health" subtitle="Calibration & MPC solve status" />
            <div className="px-5 pb-5">
              <div className="flex flex-col gap-2.5">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-2 text-[13px] text-ink-700 dark:text-ink-200">
                    <Zap size={15} className="text-amber-500" /> MPC solved rooms
                  </span>
                  <span className="text-[12px] font-medium text-ink-900 dark:text-white">{loading ? "—" : String(mpcRooms.length)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-2 text-[13px] text-ink-700 dark:text-ink-200">
                    <HeartPulse size={15} className="text-teal-500" /> Latest MPC solve
                  </span>
                  <span className="text-[12px] font-medium text-ink-900 dark:text-white">
                    {loading ? "—" : latestMpcSolve ? timeAgo(latestMpcSolve) : "—"}
                  </span>
                </div>
              </div>
            </div>
              ...
              {(report?.comfortLeaderboard ?? []).slice(0, 4).map((r, i) => (
                <div key={r.roomId} className="flex items-center justify-between py-1.5 text-[13px]">
                  <div className="flex items-center gap-2">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary-50 text-[11px] font-medium text-primary-700 dark:bg-primary-900/40 dark:text-primary-300">{i + 1}</span>
                    <span className="truncate font-medium text-ink-700 dark:text-ink-200">{r.roomLabel}</span>
                  </div>
                  <span className={clsx("tabular-nums", r.deviationC > 2 ? "text-red-600 dark:text-red-300" : r.deviationC > 1 ? "text-amber-600 dark:text-amber-400" : "text-teal-600 dark:text-teal-300")}>
                    Δ {r.deviationC.toFixed(1)}°C
                  </span>
                </div>
              ))}
              {(report?.comfortLeaderboard ?? []).length === 0 && !loading && (
                <p className="py-4 text-center text-[12px] text-ink-400">No instrumented rooms with readings yet.</p>
              )}
            </div>
          </Card>
        </div>
      ...
      <Card className="mt-5">
        ...
        <div className="divide-y divide-ink-100 dark:divide-ink-800">
          {loading && <p className="px-5 py-6 text-center text-[13px] text-ink-400">Loading anomalies…</p>}
          {!loading && recentAnomalies.length === 0 && (
            <p className="px-5 py-6 text-center text-[13px] text-ink-400">No anomalies recorded yet.</p>
          )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-ink-400">
                    {formatKey(a.anomalyType)} · {a.residualC !== null && a.thresholdC !== null ? `Δ ${a.residualC.toFixed(2)}°C vs ${a.thresholdC.toFixed(2)}°C` : "opened " + new Date(a.openedAt).toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                {a.cause && <span className="hidden text-[12px] text-ink-400 sm:inline">{formatKey(a.cause)}</span>}
