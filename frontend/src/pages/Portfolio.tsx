...
function BuildingCard({
  ...
  building: Building;
  onRequestDelete: (building: Building) => void;
}) {
    ...
    <Card className="flex flex-col p-5 transition duration-200 hover:-translate-y-1 hover:shadow-lg hover:border-primary-300">
      <div className="flex items-start justify-between">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            onClick={() => onRequestDelete(building)}
            className="flex h-9 w-9 items-center justify-center rounded-xl text-ink-400 transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950 dark:hover:text-red-400"
          ...
          <Link
            to={base}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary-500 px-4 py-2.5 text-[14px] font-medium text-white transition hover:bg-primary-600 active:bg-primary-700"
          >
            View details
          </Link>
        </div>
...
export default function Portfolio() {
  ...
  function handleConfirmDelete() {
    ...
    setDeleting(true);
    setDeleteError(null);
    deleteBuilding(pendingDelete.id)
      .then(() => {
        setBuildings((prev) => prev.filter((b) => b.id !== pendingDelete.id));
        setPendingDelete(null);
      })
      .catch((err) => {
        setDeleteError(
          err instanceof ApiError ? err.message : "Could not delete this building.",
        );
      })
    ...
    <div className="mx-auto max-w-6xl">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          {buildings.map((b) => (
            <BuildingCard key={b.id} building={b} onRequestDelete={setPendingDelete} />
          ))}
        </div>
      ) : (
        <div className="flex min-h-[280px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-ink-200 bg-white/50 px-6 text-center dark:border-ink-700 dark:bg-ink-900/50">
      ...
      <ConfirmDialog
        ...
        title={`Delete ${pendingDelete?.name ?? "this building"}?`}
        message={
          <>
            <p>
              This permanently deletes the building, its floors and rooms, and all
              sensor history, calibration data, and schedules for it. This cannot be
              undone.
            </p>
            {deleteError && (
